...
impl RawClient {
    ...
    pub fn emit_ack<D>(&self, id: Option<i32>, data: D) -> Result<()>
    where
        D: Into<Payload>,
    {
        let id = match id {
    ...
    fn handle_event(&self, packet: &Packet) -> Result<()> {
        ...
        if let Ok(Value::Array(contents)) = serde_json::from_str::<Value>(data) {
            ...
            self.callback(&event, data.to_string(), packet.id)?;
        }
...
mod test {
    ...
    fn socket_io_integration() -> Result<()> {
        ...
        let ack_callback = move |message: Payload, socket: RawClient, _| {
